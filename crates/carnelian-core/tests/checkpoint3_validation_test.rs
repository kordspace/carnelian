...
async fn test_cross_instance_memory_export_import_roundtrip() {
    let container = create_postgres_container().start().await.expect("Failed to start container");
    ...
    assert!(import_result.memory_id != Uuid::nil(), "Imported memory should have valid ID");
...
async fn test_chain_anchor_published_and_verified() {
    ...
    assert_eq!(proof.get("anchor_id").and_then(|v| v.as_str()), Some(anchor_id.as_str()));
    
    // Verify hash
    let hash = proof.get("hash").and_then(|v| v.as_str()).expect("Hash should exist");
...
async fn test_fresh_install_init_and_health_check() {
    ...
    assert!(migration_count >= 14, "All migrations including new ones should be applied");
...
async fn test_complete_feature_set_task_execution() {
    ...
    {
        let mut sched = scheduler.lock().await;
        sched.shutdown().await.expect("Failed to shutdown scheduler");
    }
...
async fn test_complete_feature_set_capability_enforcement() {
    ...
    let has_capability = policy_engine
        .check_capability("identity", &identity_id.to_string(), "test.capability", None)
        .await
...
async fn test_complete_feature_set_ledger_integrity() {
    ...
    let ledger = Ledger::new(pool.clone());
    ledger.load_last_hash().await.expect("Failed to load last hash");
    ...
    let verified = ledger.verify_chain(None).await.expect("Failed to verify chain");
    assert!(verified, "Ledger chain should verify");
    
    // Check events exist
    let recent_events = ledger.get_recent_events(5).await.expect("Failed to get recent events");
...
async fn test_security_safe_mode_blocks_side_effects() {
    ...
    let ledger = Ledger::new(pool.clone());
    let safe_mode_guard = carnelian_core::safe_mode::SafeModeGuard::new(pool.clone(), Arc::new(ledger));
    ...
    safe_mode_guard.enable(None, None).await.expect("Failed to enable safe mode");
    
    let is_enabled = safe_mode_guard.is_enabled().await;
    assert!(is_enabled, "Safe mode should be enabled");
    
    // Disable safe mode
    safe_mode_guard.disable(None).await.expect("Failed to disable safe mode");
...
async fn test_performance_task_execution_latency_under_2s() {
    ...
    assert!(elapsed < Duration::from_secs(2), "Task creation should complete under 2 seconds");
...
async fn test_performance_heartbeat_under_5s() {
    ...
    assert!(elapsed < Duration::from_secs(5), "Heartbeat should complete under 5 seconds");
