...
impl SkillBook {
    ...
    pub fn load_categories(&self) -> carnelian_common::Result<Vec<CategoryInfo>> {
        ...
        let content = std::fs::read_to_string(&categories_path).map_err(|e| {
            carnelian_common::Error::Config(format!("Failed to read categories.json: {}", e))
        })?;

        let definition: CategoriesDefinition = serde_json::from_str(&content).map_err(|e| {
            carnelian_common::Error::Config(format!("Failed to parse categories.json: {}", e))
        })?;
    ...
    pub fn load_catalog(
        &self,
    ) -> carnelian_common::Result<carnelian_common::types::SkillBookCatalog> {
            ...
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    ...
                    if let Ok(manifest) = self.load_manifest(&manifest_path) {
                        ...
                        let required_config = manifest
                            .required_config
                            .iter()
                            .map(|f| carnelian_common::types::SkillConfigField {
                                key: f.key.clone(),
                                label: f.label.clone(),
                                secret: f.secret,
                            })
    ...
    fn load_manifest(&self, path: &Path) -> carnelian_common::Result<SkillBookManifest> {
        let content = std::fs::read_to_string(path).map_err(|e| {
            carnelian_common::Error::Config(format!("Failed to read manifest: {}", e))
        })?;

        let manifest: SkillBookManifest = serde_json::from_str(&content).map_err(|e| {
            carnelian_common::Error::Config(format!("Failed to parse manifest: {}", e))
        })?;
    ...
    pub fn get_entry(
        &self,
        skill_id: &str,
    ) -> carnelian_common::Result<carnelian_common::types::SkillBookEntry> {
        let skill_path = self.skill_book_path.join(skill_id);
        let manifest_path = skill_path.join("skill-book.json");

        if !manifest_path.exists() {
            return Err(carnelian_common::Error::Validation(format!(
                "Skill '{}' not found in Skill Book",
                skill_id
            )));
        }
    ...
    pub async fn activate(
        ...
        if !target_path.exists() {
            std::fs::create_dir_all(&target_path).map_err(|e| {
                carnelian_common::Error::Config(format!("Failed to create registry dir: {}", e))
            })?;
        }
        ...
        std::fs::write(&skill_json_path, skill_content.to_string()).map_err(|e| {
            carnelian_common::Error::Config(format!("Failed to write skill.json: {}", e))
        })?;
    ...
    pub async fn deactivate(
        &self,
        skill_id: &str,
    ) -> carnelian_common::Result<carnelian_common::types::DeactivateSkillResponse> {
        let target_path = self.registry_path.join(skill_id);

        if target_path.exists() {
            std::fs::remove_dir_all(&target_path).map_err(|e| {
                carnelian_common::Error::Config(format!("Failed to remove skill dir: {}", e))
            })?;
        }
