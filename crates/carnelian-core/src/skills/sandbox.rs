...
#[cfg(unix)]
fn apply_unix_limits(_cmd: &mut Command, limits: &ResourceLimits) -> Result<()> {
    // Note: Resource limits are enforced at the container/cgroup level
    // rather than using unsafe pre_exec hooks. This provides better
    // isolation and compatibility with the forbid(unsafe_code) policy.
    tracing::debug!(
        max_memory_mb = limits.max_memory_mb,
        max_cpu_seconds = limits.max_cpu_seconds,
        max_processes = limits.max_processes,
        "Resource limits configured (enforced by container runtime)"
    );
    Ok(())
}
