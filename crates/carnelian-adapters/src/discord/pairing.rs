//! Discord pairing flow for the `!pair` command.
//!
//! Implements a token-based pairing flow similar to the Telegram adapter:
...
async fn initiate_pairing(
    ...
    let pairing = PairingRequest::new(
        crate::types::ChannelType::Discord,
...
async fn complete_pairing(
    ...
    let session = match channel_db::get_channel_session(db_pool, "discord", channel_id).await? {
        Some(s) => s,
        None => {
            msg.channel_id
    ...
    if stored_token != token_str {
        msg.channel_id
            .say(&ctx.http, "❌ Invalid pairing token.")
            .await?;
        return Ok(());
    }
    ...
    for cap in trust_level.capabilities() {
            ...
            .await
        {
            tracing::warn!(
                error = %e,
                capability = %cap,
                "Failed to grant capability during Discord pairing"
            );
        }
    }
        ...
        .say(
            ...
            format!(
                "✅ **Pairing complete!**\n\n\
                 Trust level: `{}`\n\
                 Capabilities: {}\n\n\
                 You can now interact with Carnelian through this channel.",
                trust_level.as_str(),
                caps_str
            ),
        )
