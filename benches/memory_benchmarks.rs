...
fn benchmark_vector_search(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            let _ = manager.create(CreateMemoryRequest {
    ...
    for limit in [10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(limit), limit, |b, &limit| {
            b.to_async(&rt).iter(|| async {
                manager.search_similar(black_box(&embedding), limit, None).await
            });
        });
    }
...
fn benchmark_memory_create(c: &mut Criterion) {
    ...
    c.bench_function("memory_create", |b| {
        b.to_async(&rt).iter(|| async {
            manager.create(black_box(CreateMemoryRequest {
...
fn benchmark_memory_list(c: &mut Criterion) {
    ...
    for limit in [10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(limit), limit, |b, &limit| {
            b.to_async(&rt).iter(|| async {
                manager.list(None, None, limit, 0).await
            });
        });
    }
        ...
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..10 {
                ...
                let handle = tokio::spawn(async move {
                    manager_clone.create(CreateMemoryRequest {
